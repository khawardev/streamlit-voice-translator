...
class VoiceTranslatorAudioManager:
    """
    Renamed class to avoid conflicts with any other AudioManager classes
    """
...
async def translator_loop():
    audio_manager = None
    try:
        audio_manager = VoiceTranslatorAudioManager(
        ...
        print(f"Error in translator_loop: {e}")
        traceback.print_exc()
    finally:
        if audio_manager:
